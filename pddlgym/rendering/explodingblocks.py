...
def get_objects_from_obs(obs):
    ...
    for obj in pile_bottoms:
        bottom_to_pile[obj] = [[obj, obj in destroyed]]
        key = obj
        while key in on_links:
            assert on_links[key] not in bottom_to_pile[obj]
            bottom_to_pile[obj].append([on_links[key], on_links[key] in destroyed])
            key = on_links[key]
...
def get_block_params(piles, width, height, table_height, robot_height):
    ...
    for pile_i, pile in enumerate(piles):
        ...
        for block_i, (name, destroyed) in enumerate(pile):
            y = table_height + block_i * block_height
            block_positions[name] = ((x, y), destroyed)
...
def draw_blocks(ax, block_width, block_height, block_positions):
    for block_name, ((x, y), destroyed) in block_positions.items():
        ...
        if destroyed:
            mymarker = ax.scatter(x + block_width / 2, y + block_height / 2, s=300, c='black', marker='x')
